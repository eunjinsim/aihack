  ...
  "dishes": [
    {
      ...
      "culturalNote": "1 short sentence with a cultural tip or interesting fact about this dish, written in ${targetLangName} (use an empty string if there is nothing noteworthy)"
    }
  ]
}
...
Now analyze the attached photo and return the JSON described above.`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      res.status(502).json({ error: 'The AI provider request failed. Check your API key.' });
      return;
    }
